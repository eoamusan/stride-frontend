...
export default function AssetRetrievalForm({ onCancel, onSubmit }) {
  const formSchema = z.object({
      ...
      authorized: z.boolean().refine((val) => val === 'true', { message: "You must confirm that this retrieval request is authorized" }),
    })
  ...
  const onFormSubmission = (values) => {
    console.log(formState, values, 'popppp')
    if (!isValid) return
    onSubmit(values)
  }
              ...
              <FormItem className="flex flex-col gap-3 items-baseline">
                <FormControl className="flex w-full">
                  <div className="flex items-center gap-3">
                    ...
                    <Label htmlFor="terms" className="text-destructive">I confirm this retrieval request is authorized.</Label>
                  </div>
                </FormControl>
                <FormMessage />
              </FormItem>
