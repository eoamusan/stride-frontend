...
export default function LocationForm({ onBack, onNext, formValues }) {
  const formSchema = z.object({
      ...
      roomOrArea: z.string().min(1, "Room/Area is required"),
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      roomOrArea: formValues.roomOrArea || '',
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col gap-3 items-baseline">
                ...
                <FormControl className="flex w-full">
                  <Input type="number" placeholder="Enter room number or area" onChange={field.onChange} value={field.value} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
