import emptyVendor from '@/assets/images/welcome.svg';
import { Button } from '@/components/ui/button';

export default function EmptyBank({ onClick }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <img src={emptyVendor} className="size-[130px]" alt="No vendors" />
      <h2 className="mt-4 text-2xl font-bold">Welcome to Banking & Reconciliation</h2>
      <p className="mt-2 mb-4 text-sm text-[#7D7D7D]">
        Your complete solution for bank account management and transaction reconciliation
      </p>
      <Button onClick={onClick} className={'h-10 w-full max-w-[298px]'}>
        Get Started
      </Button>
    </div>
  );
}
